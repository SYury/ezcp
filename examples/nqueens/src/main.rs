...
fn read_int() -> usize {
    ...
    io::stdin()
        ...
        .expect("No input!");
    input_line.trim().parse().expect("Input is not a valid unsigned integer!")
}
...
fn main() {
    ...
    for i in 0..n {
        ...
        diag2.push(solver.new_variable(-(i as i64), (n as i64) - 1 - (i as i64), format!("-diag_{}", i)));
        let d1 = Box::new(SimpleArithmeticConstraint::new(diag1[i].clone(), vars[i].clone(), i as i64, false));
        solver.add_constraint(d1);
        let d2 = Box::new(SimpleArithmeticConstraint::new(diag2[i].clone(), vars[i].clone(), -(i as i64), false));
